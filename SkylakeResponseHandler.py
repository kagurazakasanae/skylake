...
class SkylakeResponseHandler(object):
    ...
    def response_handler(self):
        ...
        for i in self.config.keys():
            ...
            if 'BIND_HOST' in self.config[i].keys() and self.config[i]['BIND_HOST'] == self.env['REQUEST_HOST']:
                ...
                elif self.config[i]['MODE'] == 'NATIVE':
                    return self.native_handler(self.config[i])
        ...
        if not os.path.isfile(document_path):
            ...
            if dir_list == False:
                ...
                try:
                    ...
                    return self.set_error(403, 'Permission denied')
        else:
    ...
    def cgi_handler(self, config):
        ...
        if not document_path.startswith(os.path.abspath(config['HOME_PATH'])):
            return self.set_error(403, 'Request Resource is not under HOME_PATH')
        if not os.path.exists(document_path):
            if 'REWRITE_TO' in config.keys() and 'REWRITE_PATH' in config.keys():
                document_path = os.path.abspath(config['HOME_PATH']) + config['REWRITE_TO']
                if not os.path.exists(document_path) or not uri.startswith(config['REWRITE_PATH']):
                    return self.set_error(404, 'File Not Found')
                else:
                    self.PATH_INFO = uri.split('?')[0]
            else:
                return self.set_error(404, 'File Not Found')
        if not os.path.isfile(document_path):
            if 'DEFAULT_PAGE' in config.keys():
    ...
    def native_handler(self, config):
        if 'DIR_LIST' not in config.keys():
            dir_list = False
        else:
            dir_list = config['DIR_LIST']
        ...
        self.PATH_INFO = ''
        envdata = {}
        envdata['SERVER_NAME'] = self.env['SERVER_NAME']
        envdata['USER'] = getuser()
        envdata['HOME'] = config['HOME_PATH']
        QUERY_STRING = ''
        if '?' in self.env['REQUEST_URI']:
            QUERY_STRING = self.env['REQUEST_URI'][self.env['REQUEST_URI'].find('?')+1:]
        envdata['QUERY_STRING'] = QUERY_STRING
        envdata['REQUEST_METHOD'] = self.env['REQUEST_METHOD']
        CONTENT_TYPE = ''
        CONTENT_LENGTH = ''
        ...
        envdata['CONTENT_TYPE'] = CONTENT_TYPE
        envdata['CONTENT_LENGTH'] = CONTENT_LENGTH
        envdata['REQUEST_URI'] = self.env['REQUEST_URI']
        envdata['REMOTE_ADDR'] = self.env['CLIENT_ADDR']
        envdata['REMOTE_PORT'] = self.env['CLIENT_PORT']
        envdata['SERVER_ADDR'] = self.env['SERVER_ADDR']
        envdata['SERVER_NAME'] = self.env['SERVER_NAME']
        envdata['HTTP_HOST'] = self.env['REQUEST_HOST']
        envdata['HTTP_USER_AGENT'] = self.env['USER-AGENT']
        envdata['HTTP_ACCEPT'] = self.env['HTTP_ACCEPT']
        envdata['HTTP_ACCEPT_ENCODING'] = self.env['HTTP_ACCEPT_ENCODING']
        envdata['HTTP_ACCEPT_LANGUAGE'] = self.env['HTTP_ACCEPT_LANGUAGE']
        envdata['HTTP_COOKIE'] = self.env['HTTP_COOKIE']
        envdata['HTTP_BODY'] = self.env['REQUEST_BODY']
        envdata['REQUEST_TIME_FLOAT'] = str(time.time())
        sys.path.append(config['HANDLER_PATH'])
        try:
            handler = __import__(config['HANDLER'])
            handled, statuscode, response_header, body = handler.handle_request(envdata)
        except Exception as e:
            return self.set_error(500, "<pre>" + str(traceback.format_exc()) + "</pre>")
        if handled:
    ...
    def cgi_file_handler(self, document_path, config, handler):
        ...
        os.putenv('SCRIPT_FILENAME', os.path.abspath(document_path))
        os.putenv('HOME', config['HOME_PATH'])
        os.putenv('PATH_INFO', self.PATH_INFO)
        ...
        os.putenv('DOCUMENT_URI', self.env['REQUEST_URI'])
